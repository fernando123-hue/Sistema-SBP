...
**Fechamento de 24/09/2026:** todos os 68 achados têm destino na tabela — corrigido com PR, refutado com motivo, decisão do dono ou conferência de implantação. Os quatro últimos (N-26, N-38, N-40, N-41) dependem da produção real e ficaram reunidos em `DECISOES.md § AT-47 — O que conferir no dia de ligar a produção`; o N-28 fechou nos PRs #114 e #115.

**Reparo de 23/09/2026 — dezesseis destinos estavam fora da tabela.** Os PRs #59
a #70 escreveram a célula "Destino" de C-05, C-07, C-08, C-09, C-10, C-13, C-14,
C-17, C-22, C-23, N-07, N-14, N-17, N-18, N-27 e N-37 **na primeira linha do
arquivo**, coladas umas nas outras, em vez de na linha do achado. A linha do
achado continuou com a célula vazia, e vazia quer dizer *pendente*: por isso
`ESTADO.md` seguiu listando C-07, C-08 e C-09 como médios em aberto por seis
dias, e uma sessão chegou a começar a corrigir de novo o que o PR #62 já tinha
corrigido. As dezesseis foram reinseridas **depois de conferir cada uma contra o
código**, não por confiar no texto que estava lá.

Por que isto importa mais do que parece: esta tabela é o único lugar que diz o
que ainda falta. Quando ela mente, o trabalho é refeito ou esquecido — os dois
defeitos que o sistema existe para curar, agora no próprio registro dele. Ao
editar uma célula, **confira que a linha do achado mudou** (`git diff` mostrando
a linha certa), nunca só que o texto entrou no arquivo.
...
| C-05 | MÉDIO | `src/adapters/fabrica.ts:52` | Nada no código impede e-mail real, com CPF, de ir a fornecedor externo sem máscara (A52) ou à camada gratuita do Gemini (A38) | **Em parte** (17/09, PR #63): caixa real recusa IA simulada e Gemini. **Falta** a máscara de CPF de `A52`, que exige medir antes |
...
| C-07 | MÉDIO | `src/adapters/ingestao-graph.ts:329` | E-mail encaminhado como anexo e anexo-link do OneDrive somem sem registro | **Mesmo defeito do N-02**, corrigido no PR #62 |
| C-08 | MÉDIO | `src/servicos/autenticacao.ts:275` | Com uma sessão de gestor roubada ou deixada aberta, o atacante toma a conta de forma permanente e tranca o gestor legítimo para fora | **Em parte** (17/09): gestor não redefine a própria senha por esta rota. **Falta decidir** (pergunta ao dono, `DECISOES.md § H.4` item 30): pedir a senha do gestor para redefinir senha, desativar ou mudar papel, e o que fazer com outro gestor |
| C-09 | MÉDIO | `src/servicos/autenticacao.ts:85` | A trava por conta pode ser furada disparando tentativas em paralelo | **Corrigido** (17/09): a tentativa é reservada, com a conta travada, antes do hash — na entrada e na troca de senha |
| C-10 | MÉDIO | `src/servicos/fila.ts:139` | Concluir, devolver e transferir o mesmo item em paralelo passam sem trava | **Corrigido** (17/09): `concluir`, `devolver` e `transferir` travam a linha do item antes de ler; `definirAtivacao` também, na mesma ordem; impasse é repetido (`servidor/conflito.ts`) |
...
| C-13 | MÉDIO | `src/servidor/http.ts:318` | Nenhuma rota autenticada confere origem; SameSite=Lax não cobre origem do mesmo site | **Corrigido** (17/09): o middleware recusa com 403 pedido que altera estado em `/api` vindo de outra origem (`Sec-Fetch-Site`, ou `Origin` sem ele). Exigir `Content-Type: application/json` em `corpoJson` ficou para depois |
| C-14 | MÉDIO | `src/servidor/http.ts:235` | Com PROXIES_CONFIAVEIS=1, o cliente escolhe a própria chave do limite por origem | **Adiado para a publicação (`A46`)**: sem proxy, `PROXIES_CONFIAVEIS=0` já usa o balde único (seguro). No dia do proxy, declarar qual cabeçalho ele escreve em vez de adivinhar pela cadeia. O teto do mapa (segunda parte) foi corrigido no N-37 |
| C-15 | BAIXO | `src/adapters/assistente-modelo.ts:111` | O assistente grava no log texto livre escolhido pelo modelo, o que abre caminho para a pergunta, com e-mail colado, chegar ao log | **Corrigido** (PR #90): o aviso grava só quantas citações foram descartadas, nunca o texto; teste com CPF sintético no `verbetesUsados` |
| C-16 | BAIXO | `src/app/api/notas/route.ts:57` | Qualquer colaborador cria notas sem limite, e toda tela de trabalho carrega todas as notas gerais sem teto | **Corrigido** (PR #91, `§ AT-44`): 20 notas por minuto por pessoa; leitura da tela com teto por faixa de relevância (resultado idêntico); listagem com teto de 200 e `truncado` |
| C-17 | BAIXO | `src/app/api/sessao/route.ts:146` | CSRF de login: outro site consegue deixar o navegador da vítima logado na conta do atacante. A única defesa CSRF das demais rotas é SameSite=Lax | **Corrigido** (17/09) com o C-13: a entrada também passa pela conferência de origem |
| C-18 | BAIXO | `src/servicos/autenticacao.ts:91` | A mensagem de conta bloqueada enumera contas ativas, e a premissa registrada no comentário está errada | **Corrigido** (PR #92 e PR #93, `§ A57`): o bloqueio espera o piso de 250 ms (respondia em 5 ms); por decisão do dono, a conta travada responde como qualquer recusa, e a tela mostra uma orientação fixa de esperar |
| C-19 | BAIXO | `src/servicos/autenticacao.ts:203` | Tentativas contra e-mail inexistente, falhas na troca de senha e recusas por bloqueio não entram na trilha | **Corrigido** (PR #94, `§ AT-45`): as quatro recusas entram na trilha da conta; e-mail inexistente vira evento sem o endereço. Alerta por volume fica anotado para depois |
| C-20 | BAIXO | `src/servicos/memoria.ts:233` | porCorrelacao contorna a lista fechada que tirou a trilha de Colaborador da consulta | **Corrigido** (PR #95): `porCorrelacao` usa a mesma lista fechada de `porEntidade`; a linha de `Colaborador` numa correlação mista não sai mais |
| C-21 | BAIXO | `src/servicos/qualidade.ts:123` | GET /api/qualidade?dias=tudo lê todas as revisões resolvidas, para qualquer papel e sem limite de taxa | **Corrigido** (PR #96): 12 consultas por minuto por pessoa. Ficam de fora, anotados: restringir `tudo` por papel (muda quem vê o quê — só com decisão do dono, e o limite já tira a multiplicação) e agregar no banco em vez de carregar as linhas (vale quando o volume crescer) |
| C-22 | BAIXO | `src/servicos/revisao.ts:148` | Resolução da mesma revisão por duas pessoas: a última sobrescreve em silêncio | **Corrigido** (17/09): `resolver` trava a linha da revisão antes de ler |
| C-23 | BAIXO | `src/servicos/revisao.ts:152` | Resolver revisão aceita categoria inativa, e o item aprovado some da distribuição e do painel | **Corrigido** (17/09): `resolver` recusa categoria desativada |
| C-24 | BAIXO | `src/servidor/http.ts:92` | Recusas de autorização (403/401/422) não deixam rastro em lugar nenhum | **Corrigido em parte** (PR #97): permissão negada (403 por papel) vira log e evento `autorizacao` com quem, papel e operação, no máximo um por pessoa e operação a cada 10 minutos. Ficam de fora, anotados: 401 sem sessão (sem pessoa para registrar; o limite por origem já existe) e recusas de negócio como "só o responsável pode concluir" (422 — separar a sondagem do engano comum pede critério por rota). **Complemento (PR #130, pendência 8):** concluir item de outra pessoa passou a deixar o mesmo rastro, com critério para o engano comum — quem já foi responsável pelo item (tela desatualizada) fica só no log. |
| C-25 | INFORMATIVO | `src/adapters/armazenamento-disco.ts:124` | Com ANEXOS_SECRET vazio (padrão do .env.example), um único segredo forja sessão e decifra os anexos, e rotacioná-lo depois de um vazamento quebra os documentos | **Corrigido em parte** (PR #98): em produção, `ANEXOS_SECRET` é obrigatório e diferente de `SESSAO_SECRET` — o sistema recusa subir. Fica anotada a rotação da sessão com duas chaves (`SESSAO_SECRET_ANTERIOR`), trabalho maior |
| C-26 | INFORMATIVO | `src/app/api/distribuicao/confirmar/route.ts:38` | O limite de confirmação usa a data enviada no corpo e muda de balde a cada data | **Corrigido** (PR #99): balde por pessoa sem a data, 30 por minuto, além do balde por data. Recusar datas futuras fica anotado — muda a operação, só com decisão do dono |
| C-27 | INFORMATIVO | `src/servicos/ingestao.ts:172` | Injeção que escapa dos dois detectores some com o trabalho ou passa aprovada; a lista de A34 ainda não existe | **Destino: `A34`, fase 4** (PR #100 corrigiu o comentário que ainda dizia "decisão do dono"). A lista é funcionalidade nova — estrutura nova (a `Revisao` exige item), tela e duas ações —, não correção pontual; até ela existir, `AT-24` segura o e-mail suspeito sem item fora da limpeza. Calibrar os limiares pela taxa de acerto medida (`A30`), e não pela confiança declarada, fica junto |
...
| N-04 | MÉDIO | `src/app/fila/page.tsx:225` | "Concluir" na Minha fila: um toque, sem confirmação e sem desfazer, com botão de 36 px no celular | **Corrigido** (PR #102, `§ AT-46`): confirmado lendo o código; Concluir pede dois toques, como o Descartar da Revisão, e o botão pequeno tem 44 px no celular em todas as telas |
| N-05 | MÉDIO | `src/app/fila/page.tsx:276` | Lista "Transferir para" inclui a própria pessoa e quem está afastado, e transferir para si mesma some com o item da tela sem aviso | **Corrigido** (PR #103): confirmado lendo o código; o servidor recusa transferir para quem já está com o item, a tela tira a própria pessoa da lista e marca quem está de férias ou ausente (marca, não bloqueia), e a escala é pedida com `hojeIso()` |
| N-06 | MÉDIO | `src/app/painel/page.tsx:489` | Painel "Por pessoa" ignora o período escolhido e não avisa | **Corrigido** (PR #104): confirmado lendo o código; Concluídos por pessoa conta só o período escolhido, Atribuídos e Pendentes levam "(hoje)" como as outras colunas de estado atual, quem só concluiu volta a aparecer e a linha única do colaborador aparece mesmo zerada |
| N-07 | MÉDIO | `src/servicos/autenticacao.ts:409` | A trava do último gestor ativo não segura no MySQL: dois gestores podem desativar um ao outro ao mesmo tempo | **Confirmado e corrigido** (17/09): as linhas dos gestores ativos são travadas antes da contagem |
...
| N-12 | MÉDIO | `src/servicos/ingestao.ts:559` | Conteúdo externo cria Liga sem limite, o índice único de Liga não protege nada e toda ingestão lê a tabela inteira de ligas | **Corrigido** (PR #89, `§ AT-43`): no máximo 3 ligas novas por e-mail (hipótese); o que passa disso fica sem liga e vai para a revisão, com evento |
...
| N-14 | MÉDIO | `src/servicos/ingestao.ts:76` | A sincronização nunca informa 'desde': com mais de 200 mensagens na Inbox, toda busca pelo Graph falha para sempre | **Confirmado** — é o mesmo defeito do C-03; corrigido no PR #59 (`AT-35`) |
...
| N-16 | MÉDIO | `src/servicos/rotinas.ts:133` | Uma única linha de Afastamento com tipo inválido suspende toda a limpeza diária (motivos, e-mails, dados de item, contagem), e a falha gravada diz só 'Error' | **Corrigido** (PR #89, `§ AT-43`): cada etapa da limpeza corre por si; a rotina ainda falha, e a mensagem diz qual etapa quebrou |
| N-17 | MÉDIO | `src/servidor/ambiente.ts:26` | Ingestão e IA simuladas são o padrão também com NODE_ENV=production | **Confirmado e corrigido** (17/09): caixa real (`INGESTAO_ADAPTER` diferente de `mock`) recusa `IA_ADAPTER` `mock` ou `gemini` |
| N-18 | MÉDIO | `src/servidor/ambiente.ts:61` | SESSAO_SECRET e BUSCA_SECRET aceitam qualquer texto com 16 caracteres, inclusive os valores públicos do CI e do vitest, e produção não tem nenhuma checagem de entropia | **Confirmado e corrigido** (17/09): em produção, `SESSAO_SECRET`, `BUSCA_SECRET` e `ANEXOS_SECRET` com valor de teste público são recusados |
...
| N-26 | BAIXO | `src/adapters/ingestao-graph.ts:121` | Remetente vem do cabeçalho From sem nenhum sinal de autenticação, e a tela o mostra como fato | **Confirmado; adiado com destino** — `DECISOES.md § AT-47`: veredito SPF/DKIM/DMARC e From ≠ Sender, com revisão e aviso na tela, implementado no dia da credencial do Graph **com cabeçalhos reais do tenant** (amostra inventada testaria o que imaginamos que a Microsoft manda) |
...
| N-28 | BAIXO | `src/app/acesso/page.tsx:133` | Mensagens em inglês, com ids internos e com notas de desenvolvimento na tela | **Confirmado; (a) corrigido** (PR #114): validação em português de gente (`core/mensagem-de-validacao.ts`, servidor e tela), permissão negada sem papel/operação/permitidos, 14 `ErroDeNegocio` sem id com varredura em teste; **(b) corrigido** (PR #115): textos de Caixa, Revisão, Distribuição, Painel e da narrativa da rodada sem justificativa de engenharia, com varredura em teste |
| N-29 | BAIXO | `src/app/acesso/page.tsx:147` | Senha provisória ainda não anotada é sobrescrita pela próxima, e "Desligar acesso" age com um clique | **Confirmado e corrigido** (PR #109): com a senha na tela, gerar outra e cadastrar ficam desabilitados, com o motivo escrito no cartão; "Desligar acesso" pede o segundo clique (religar não) |
...
| N-32 | BAIXO | `src/componentes/matrizes.tsx:228` | Aviso sempre com role="alert", campos sem rótulo e alvos de 36 px no celular | **Confirmado e corrigido**: (a) e (b) no PR #110 — `Aviso` é `alert` só no tom de alerta, `status` nos demais, e o neutro tem cor própria; datas do Painel e campos extras da Revisão com `aria-label`. (c) já no PR #102 (N-04): `Botao` pequeno com 44 px no celular |
| N-33 | BAIXO | `src/servicos/distribuicao.ts:835` | Distribuição concorrente com a desativação de uma pessoa pode entregar itens a quem acabou de ser desligado, e eles ficam invisíveis | **Confirmado por teste e corrigido** (PR #111): reproduzido na `sbp_teste` (7 atribuições ativas para a pessoa desligada); `confirmar` trava em modo compartilhado as linhas de quem está ativo antes de planejar, e a desativação concorrente espera e devolve o que a distribuição gravou |
| N-34 | BAIXO | `src/servicos/revisao.ts:110` | A fila de revisão carrega o corpo inteiro (LongText) de até 100 e-mails a cada abertura e não usa | **Confirmado e corrigido** (PR #106): `listarPendentes` seleciona só remetente e assunto; teste confere as chaves que voltam do banco |
| N-35 | BAIXO | `src/servicos/revisao.ts:174` | Payload ilegível vira padrão vazio na revisão e é sobrescrito, apagando campos da IA e liga mencionada sem log | **Confirmado e corrigido** (PR #106): payload ilegível falha alto (500 com correlação), a transação aborta e o item fica em revisão como estava; a mensagem leva o id, nunca o conteúdo |
...
| N-38 | INFORMATIVO | `src/adapters/ingestao-graph.ts:176` | O token de aplicativo `.default` alcança toda caixa que a permissão permitir, e o código não tem como limitar | **Confirmado; conferência obrigatória na implantação** — `DECISOES.md § AT-47`: provar que outra caixa dá 403 com o mesmo token (`Test-ApplicationAccessPolicy`), certificado em vez de segredo, validade anotada; sem o 403 a ingestão não liga |
| N-39 | INFORMATIVO | `src/app/api/painel/route.ts:33` | Colaborador recebe números da equipe inteira por categoria, a qualidade da IA e a contagem de itens por liga | **Confirmado; corrigido em parte e perguntado** (PR #112): a contagem por liga do seletor da Caixa segue o recorte de `A24` (bate com a lista); se o colaborador vê os totais do setor no Painel é decisão do dono — `DECISOES.md § H.4` item 34, **respondida: opção (a), `§ A58`** (continua vendo os totais, sem nome de ninguém) |
| N-40 | INFORMATIVO | `src/core/seguranca/assinatura-de-arquivo.ts:43` | A assinatura de .docx/.xlsx aceita qualquer ZIP, .txt/.csv não têm verificação e `hash` nunca é preenchido | **Confirmado; sem superfície hoje** (nenhuma rota entrega anexo) — `DECISOES.md § AT-47`: o PR que criar download traz junto `[Content_Types].xml` sem macro, `attachment` + `nosniff`, SHA-256 em `Anexo.hash`, antivírus, recorte `A24` e auditoria de cada download |
| N-41 | INFORMATIVO | `src/servidor/cpf-protegido.ts:36` | O CPF protegido é um HMAC correto, mas um dump do banco junto com o BUSCA_SECRET revela todos os CPFs | **Confirmado; controle de implantação** — `DECISOES.md § AT-47` e `A46`: `BUSCA_SECRET` só no gestor de segredos, backup sem `.env`, restauração de teste que não sobe sem o gestor |
