...
| C-25 | INFORMATIVO | `src/adapters/armazenamento-disco.ts:124` | Com ANEXOS_SECRET vazio (padrão do .env.example), um único segredo forja sessão e decifra os anexos, e rotacioná-lo depois de um vazamento quebra os documentos | **Corrigido em parte** (PR #PRNUM): em produção, `ANEXOS_SECRET` é obrigatório e diferente de `SESSAO_SECRET` — o sistema recusa subir. Fica anotada a rotação da sessão com duas chaves (`SESSAO_SECRET_ANTERIOR`), trabalho maior |
