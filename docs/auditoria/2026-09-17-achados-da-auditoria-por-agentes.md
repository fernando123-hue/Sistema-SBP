...
| N-12 | MÉDIO | `src/servicos/ingestao.ts:559` | Conteúdo externo cria Liga sem limite, o índice único de Liga não protege nada e toda ingestão lê a tabela inteira de ligas | **Corrigido** (PR #PRNUM, `§ AT-43`): no máximo 3 ligas novas por e-mail (hipótese); o que passa disso fica sem liga e vai para a revisão, com evento |
...
| N-16 | MÉDIO | `src/servicos/rotinas.ts:133` | Uma única linha de Afastamento com tipo inválido suspende toda a limpeza diária (motivos, e-mails, dados de item, contagem), e a falha gravada diz só 'Error' | **Corrigido** (PR #PRNUM, `§ AT-43`): cada etapa da limpeza corre por si; a rotina ainda falha, e a mensagem diz qual etapa quebrou |
