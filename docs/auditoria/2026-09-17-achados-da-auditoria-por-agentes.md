...
| C-27 | INFORMATIVO | `src/servicos/ingestao.ts:172` | Injeção que escapa dos dois detectores some com o trabalho ou passa aprovada; a lista de A34 ainda não existe | **Destino: `A34`, fase 4** (PR #PRNUM corrigiu o comentário que ainda dizia "decisão do dono"). A lista é funcionalidade nova — estrutura nova (a `Revisao` exige item), tela e duas ações —, não correção pontual; até ela existir, `AT-24` segura o e-mail suspeito sem item fora da limpeza. Calibrar os limiares pela taxa de acerto medida (`A30`), e não pela confiança declarada, fica junto |
