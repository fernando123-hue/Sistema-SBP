| **Confirmado e corrigido** (22/09): a chave do anexo passa a ser montada com `/` cravado, nunca com `join`. No Windows ela saía com ``, e numa migração para servidor Linux nenhuma chave acharia o arquivo — leitura devolvendo "ausente" e expurgo apagando o nada, os dois calados. Chave antiga com `` continua sendo lida. || **Confirmado e corrigido** (22/09): `anexos:conferir` não responde mais "0 em texto puro" quando não consegue ler a pasta. Pasta inexistente continua valendo zero (instalação nova); qualquer outra falha sobe com o caminho e o motivo. || **Confirmado e corrigido** (22/09): `db:limpar` remove os BYTES dos anexos antes de apagar as linhas. Antes, a cascata apagava `Anexo` e os arquivos ficavam no disco sem referência, fora do alcance do expurgo. || **Confirmado e corrigido** (22/09): `Atribuicao`, `Execucao`, `JustificativaDeAtribuicao` e `Revisao` passaram de `Cascade` para `Restrict` (migração `historico_nao_cascateia`). Um `DELETE` no `Item` levava o histórico junto, em silêncio. Três testes, um deles conferindo a regra direto no `information_schema`. || **Confirmado e corrigido** (22/09): `@prisma/adapter-better-sqlite3` removido do `package.json` e do `serverExternalPackages`. Eram 12 MB de binário e um script de instalação sem nenhum uso no código. `npm run build` verde sem ele. || **Confirmado e corrigido** (22/09): todas as actions fixadas por SHA com a versão em comentário (inclusive a de terceiro que recebe o `GITHUB_TOKEN`); `persist-credentials: false` em todo checkout (nenhum job empurra nada); e `npm ci --ignore-scripts` no job de auditoria — auditar dependência não exige executá-la. || **Confirmado e corrigido** (17/09): `conferirSenha` passou a distinguir `hash_ilegivel` de `nao_confere`. Hash ilegível não gasta tentativa da pessoa, devolve a tentativa reservada, grava evento para um humano e lança `CredencialIlegivelError` (503, mensagem pública sem id). O piso de resposta continua pago, para o caminho novo não virar oráculo. || **Confirmado e corrigido em duas camadas** (17/09): varredura (`trilha-append-only.test.ts`) impede `update`/`delete`/`upsert` em `LogAuditoria` e `EventoProcessamento` no código de produção — exceção nominal para `db:limpar` —, e a migração `trilha_append_only` põe TRIGGER no MySQL que recusa qualquer `UPDATE`. DELETE fica para o privilégio do usuário do banco na implantação. || **Confirmado e corrigido** (17/09): os testes comparavam datas por conta própria; agora chamam `perfilAtual` com o cookie real. Provado por mutação: removida a conferência em `sessao.ts`, o teste fica vermelho. || **Confirmado e corrigido** (17/09): `src/servicos/permissoes.test.ts` — mapa de 12 operações sensíveis com papel insuficiente, mais o contraponto da própria fila. Apagar um `exigirPapel` de serviço agora fica vermelho (provado por mutação). || **Confirmado MEDINDO e corrigido** (17/09): com o `upsert`, a segunda transação esperava a trava e ainda lia o crédito velho (REPEATABLE READ tira a fotografia no SELECT que o `upsert` faz antes de travar); e duas PRIMEIRAS distribuições do dia estouravam unicidade crua. Agora a trava é `INSERT ... ON DUPLICATE KEY UPDATE` (`tomarTravaDoDia`), com dois testes de concorrência real. || **Confirmado e corrigido** (17/09): troca de senha, senha provisória e destravamento gravam fato e trilha na MESMA transação (`transacaoComNovaTentativa`); o hash fica fora dela, para scrypt não segurar linha travada. Teste derruba a trilha de dentro do `tx` e prova que a senha volta atrás. |**Adiado para a publicação (`A46`)**: sem proxy, `PROXIES_CONFIAVEIS=0` já usa o balde único (seguro). No dia do proxy, declarar qual cabeçalho ele escreve em vez de adivinhar pela cadeia. O teto do mapa (segunda parte) foi corrigido no N-37 |**Confirmado e corrigido** (17/09): acima do teto, depois das vencidas, saem as chaves mais antigas; primeiro teste do limitador |**Corrigido** (17/09) com o C-13: a entrada também passa pela conferência de origem |**Corrigido** (17/09): o middleware recusa com 403 pedido que altera estado em `/api` vindo de outra origem (`Sec-Fetch-Site`, ou `Origin` sem ele). Exigir `Content-Type: application/json` em `corpoJson` ficou para depois |**Confirmado e corrigido** (17/09): as linhas dos gestores ativos são travadas antes da contagem |**Corrigido** (17/09): a tentativa é reservada, com a conta travada, antes do hash — na entrada e na troca de senha |**Em parte** (17/09): gestor não redefine a própria senha por esta rota. **Falta decidir** (pergunta ao dono, `DECISOES.md § H.4` item 30): pedir a senha do gestor para redefinir senha, desativar ou mudar papel, e o que fazer com outro gestor |**Corrigido** (17/09): `resolver` recusa categoria desativada |**Corrigido** (17/09): `resolver` trava a linha da revisão antes de ler |**Corrigido** (17/09): `concluir`, `devolver` e `transferir` travam a linha do item antes de ler; `definirAtivacao` também, na mesma ordem; impasse é repetido (`servidor/conflito.ts`) |**Mesmo defeito do N-02**, corrigido no PR #62 |**Em parte** (17/09, PR #63): caixa real recusa IA simulada e Gemini. **Falta** a máscara de CPF de `A52`, que exige medir antes |**Confirmado e corrigido** (17/09): em produção, `SESSAO_SECRET`, `BUSCA_SECRET` e `ANEXOS_SECRET` com valor de teste público são recusados |**Confirmado e corrigido** (17/09): caixa real (`INGESTAO_ADAPTER` diferente de `mock`) recusa `IA_ADAPTER` `mock` ou `gemini` |**Confirmado e corrigido** (17/09): a listagem vem sem bytes, só o arquivo que cabe é baixado, e o tamanho que vale é o dos bytes |**Confirmado** — é o mesmo defeito do C-03; corrigido no PR #59 (`AT-35`) |**Confirmado e corrigido** (17/09): anexo que não é arquivo entra como recusado, com motivo, e o item vai para revisão |# Achados da auditoria por agentes — 16 e 17/09/2026
