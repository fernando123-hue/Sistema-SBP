...
| C-20 | BAIXO | `src/servicos/memoria.ts:233` | porCorrelacao contorna a lista fechada que tirou a trilha de Colaborador da consulta | **Corrigido** (PR #PRNUM): `porCorrelacao` usa a mesma lista fechada de `porEntidade`; a linha de `Colaborador` numa correlação mista não sai mais |
