...
| C-26 | INFORMATIVO | `src/app/api/distribuicao/confirmar/route.ts:38` | O limite de confirmação usa a data enviada no corpo e muda de balde a cada data | **Corrigido** (PR #PRNUM): balde por pessoa sem a data, 30 por minuto, além do balde por data. Recusar datas futuras fica anotado — muda a operação, só com decisão do dono |
