...
> 5. ~~Troca de rótulo "Concluir" → "Confirmar: concluir" sem anúncio a leitor de tela~~ — **resolvida no código no #128; falta ouvir com NVDA, junto da 6.** `Anuncio` (região `aria-live="polite"` sempre presente, vazia até o primeiro clique) na Minha fila e na Revisão: diz qual item está armado (posição na tela e título — títulos se repetem) e, depois, "Item concluído/aprovado/descartado". Sem leitor de tela nesta máquina, foi conferido só na árvore de acessibilidade do Chromium.
