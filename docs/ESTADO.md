...
> 6. Avisos `role="status"` que surgem na carga do Painel, Revisão e Caixa: testar com leitor de tela (NVDA) se viram rajada de anúncios (revisão do #110). Incluir no teste: o `Anuncio` da Minha fila e da Revisão (#128) e os selos com dica escondida da Caixa (#126 — conferir se a dica é lida duas vezes).
