...
> 5. ~~Troca de rótulo "Concluir" → "Confirmar: concluir" sem anúncio a leitor de tela~~ — **resolvida no código no #128; falta ouvir com NVDA, junto da 6.** `Anuncio` (região `aria-live="polite"` sempre presente, vazia até o primeiro clique) na Minha fila e na Revisão: diz qual item está armado ("o item 8 de 48" — nunca o título, que vem do e-mail: `DECISOES.md § AT-48`) e, depois, "Item concluído/aprovado/descartado". Sem leitor de tela nesta máquina, foi conferido só na árvore de acessibilidade do Chromium.
> 6. Avisos `role="status"` que surgem na carga do Painel, Revisão e Caixa: testar com leitor de tela (NVDA) se viram rajada de anúncios (revisão do #110). Incluir no teste: o `Anuncio` da Minha fila e da Revisão (#128 — "item 8 de 48" basta para se orientar? duas aprovações seguidas são lidas duas vezes? na Revisão, "de N" é a lista carregada, não o total) e os selos com dica escondida da Caixa (#126 — conferir se a dica é lida duas vezes).
...
>
> *F. Novas em 26/09 (revisão do #133):*
> 27. **Outros testes com a mesma bomba de meia-noite do #133.** A causa comum é esta: `DATA_BASE` fica fixa quando o arquivo carrega, e algo nasce com a hora real (`@default(now())` do MySQL, `new Date()` no serviço). **Classe A:** item criado sem `criadoEm` e depois `confirmar({ data: DATA_BASE })`. O item cai fora do corte `criadoEm <= fimDoDia(data)` e a rodada sai vazia. Afeta `peso-e-limiar.test.ts` (≈52, ≈74), `itens.test.ts` (≈189), `pipeline.test.ts` (≈1004) e `afastamentos.test.ts` (≈181, 238, 288, 351). **Classe B:** `idade.test.ts` (`envelhecer` parte de `DATA_BASE`, mas `painel.ts` mede com `hojeIso()`). O remédio é o do #133: tirar a data do que foi gravado (`paraDataIso(item.criadoEm)`). A raiz, um relógio injetável nos serviços que gravam data, vai para `DECISOES.md § C` quando alguém atacar esta pendência.
...
>
> **2. Feito hoje (24/09), um PR por achado, cada um com teste visto vermelho, revisão por agente publicada no PR e CI lido check a check:**
> - #89 N-12 e N-16 (código solto de 22/09 recuperado) · #90 C-15 · #91 C-16 · #92 e #93 C-18 (**decisão do dono: `§ A57`, opção b**) · #94 C-19 · #95 C-20 · #96 C-21 · #97 C-24 · #98 C-25 · #99 C-26 · #100 C-27 (destino `A34`, fase 4).
> - **Todos os achados C- têm destino.** Detalhe em `DECISOES.md § AT-43, AT-44, AT-45, A57` e na tabela da auditoria.
> - As revisões por agente acharam defeito real em quase todo PR (ramo morto, evento falso, consulta sem domínio, balde de limite despejável, segredo derivado aceito, comentário que descrevia fluxo inexistente). Todos corrigidos antes de mesclar.
>
> **3. PRÓXIMO PASSO — terminar a auditoria (pedido do dono: "completar a auditoria para melhores resultados"; ele tem instruções novas depois disso).** Faltam **17 achados N- sem verificação**, na tabela `docs/auditoria/2026-09-17-achados-da-auditoria-por-agentes.md`:
> - **Médios primeiro:** N-03, N-04, N-05, N-06.
> - Baixos: N-26, N-28, N-29, N-30, N-31, N-32, N-33, N-34, N-35.
> - Informativos: N-38, N-39, N-40, N-41.
>
> Um por vez: ler o código, **confirmar ou refutar** (refutado também vira destino, com o motivo), e o confirmado vira PR com teste vermelho. Ao preencher a tabela, conferir no `git diff` que mudou a **linha do achado** (armadilha registrada lá em cima do arquivo; nesta sessão ela quase se repetiu com um regex).
>
> **4. Anotado nesta sessão para tratar (não urgente):**
> - Sondagem **horizontal** invisível: `concluir()` em `servicos/fila.ts` recusa quem não é o responsável com 422, sem rastro (revisão de segurança do #97).
> - `/api/painel` e `/api/memoria` sem limite por pessoa (revisão do #96).
> - Erro de *transporte* da IA grava a mensagem crua do fornecedor no log — conferir se algum SDK ecoa o pedido (revisão de segurança do #90).
> - Alerta por volume das recusas de entrada (`AT-45`); índice `(situacao, etapa, referencia)` em `EventoProcessamento` quando o volume justificar; rotação da sessão com duas chaves (`SESSAO_SECRET_ANTERIOR`, C-25).
> - `A34` (lista de e-mails suspeitos sem item) é a fase 4.
>
> **5. Decisões abertas com o dono:** `§ H.4` itens **30** (senha do gestor de novo em ações sensíveis) e **32** (teto de 3 tentativas do `AT-41`); `AT-42` (trava de gasto que não dependa do banco, antes de chave paga); e os antigos (29, 31, TI da associação, `A46`, `A21`, `A32`, `A44(i)`). Opcionais que viraram anotação, só com decisão dele: recusar datas futuras na distribuição (C-26) e restringir `?dias=tudo` da qualidade por papel (C-21).
>
> **6. Armadilhas desta máquina vistas hoje:** `npm run verificar` às vezes perde um worker (`spawn EPERM`) e mostra um arquivo a menos — repetir, não contar como verde. O job *Processo* fica vermelho enquanto o corpo do PR não tem o link da revisão, e verde na execução seguinte à edição — conferir o horário das duas. O gitleaks acusa literal de teste com cara de chave: gerar com `randomUUID()`; se já foi commitado, exceção por impressão digital no `.gitleaksignore` (push forçado é bloqueado).
>
...
>
> **5. C-15 corrigido e mesclado** (#90, `ab7c9ac`): o aviso de citação inventada do assistente grava só a contagem, não o texto que o modelo escreveu. **C-16 corrigido e mesclado** (#91, `48a6496`, `§ AT-44`): limite para escrever nota e teto para ler. **C-18 corrigido**: o piso de tempo (#92, `c7f4907`) e, com a resposta do dono ao `§ H.4` item 33 (opção b, `§ A57`), a mensagem única com orientação fixa na tela (#93, `f53a339`, vista rodando). **C-19 corrigido** (#94, `52f72a9`, `§ AT-45`): toda recusa de entrada deixa rastro, com teto por janela; alerta por volume anotado para depois. **C-20 corrigido** (#95, `1a7d67c`): a consulta por correlação usa a mesma lista fechada da consulta por entidade. **C-21 corrigido** (#96, `0131761`): 12 consultas de qualidade por minuto por pessoa. **C-24 corrigido em parte** (#97, `f6b59d9`): permissão negada deixa rastro, com teto por janela; 401 e recusas de negócio anotados. **Da revisão de segurança do #97, a tratar:** (i) ~~sondagem **horizontal** continua invisível~~ (resolvida no #130) — `concluir()` em `servicos/fila.ts` recusa quem não é o responsável com `ErroDeNegocio` (422), que nunca passa por `PermissaoNegadaError`; é o próximo candidato a rastro, com critério para não registrar o engano comum; (ii) a consulta de teto (aqui e no C-19) tem corrida entre ler e gravar — pode sair uma linha a mais por janela, aceito; (iii) índice `(situacao, etapa, referencia)` em `EventoProcessamento` quando o volume justificar. **Novo, da revisão técnica do #96, a verificar:** `/api/painel` e `/api/memoria` também são leituras analíticas sem limite por pessoa (mesma família do C-21). Baixos confirmados: todos com destino. **C-25 corrigido em parte** (#98, `0b0bd9d`): produção exige `ANEXOS_SECRET` próprio, nem igual nem contido no de sessão. **C-26 corrigido** (#99, `f8a1b40`): confirmar distribuição tem balde por pessoa sem a data, em compartimento próprio do limitador. Anotado da revisão de segurança: é teto por minuto, não por dia (~43 mil chamadas/dia possíveis por conta de operador) — aceitável para a equipe de hoje. **C-27 com destino**: `A34` (fase 4); comentário desatualizado corrigido. **Todos os C- têm destino.** Resta verificar os 17 N- sem verificação (lista no primeiro bloco). **Novo, da revisão de segurança do #90, ainda sem verificar (BAIXO sugerido):** em `assistente-modelo.ts` (≈170-177) e `ia-estruturada.ts`, erro de *transporte* grava a mensagem crua do fornecedor no log — seguro só se nenhum SDK ecoar o pedido (pergunta, e-mail) na mensagem de erro. Conferir fornecedor por fornecedor antes de decidir.
>
...
>
> **2. PR #84 mesclado antes dele** — `.claude/worktrees/` (pasta de um agente revisor em worktree isolado) entrou no `.gitignore`, mesma razão de `.claude/reviews/`. Nível 3 só pelo arquivo tocado (`.gitignore`); revisão técnica e de segurança publicadas, as duas aprovando.
>
...
>
> **6. Lição desta sessão:** uma sessão anterior mesclou PRs (#77, #82) sem nunca atualizar este arquivo — o dono perdeu acesso ao computador dela no meio do trabalho, e o branch de saída que ela registrou (`fix/auditoria-conteudo-externo`) nunca chegou a ser enviado ao GitHub. **Se este arquivo disser uma coisa e `git log --oneline -20` disser outra, o `git log` vence.** Atualizar este bloco faz parte de terminar o trabalho, não é opcional para depois.
>
> ### 17/09/2026, noite — PR #75 MESCLADO (`88eb390`); o próximo era C-11/N-13 *(bloco antigo, mantido como registro)*
>
