...
| A57 | Conta travada responde como qualquer recusa *(24/09/2026)* | **Decisão do dono, `§ H.4` item 33, opção (b):** depois de 5 senhas erradas a conta continua travada, mas a resposta é a mesma de qualquer recusa ("E-mail ou senha incorretos."), e a tela de entrada mostra, abaixo de toda recusa de e-mail e senha, a frase fixa *"Errou várias vezes? Espere um minuto antes de tentar de novo."*. Motivo: a mensagem específica ("volta a aceitar entrada em 30s") só aparecia para conta real, então seis tentativas contra qualquer e-mail revelavam quem tem acesso (achado C-18). A troca de senha, que exige estar logado, mantém a mensagem específica — ali não há o que enumerar. Implementado em `servicos/autenticacao.ts` e `app/entrar/page.tsx`. |
| A58 | Colaborador continua vendo os totais do setor no Painel *(24/09/2026)* | **Decisão do dono, `§ H.4` item 34, opção (a):** quem é colaborador segue vendo no Painel os números do setor inteiro por categoria (abertos, concluídos, pendentes) e a taxa de acerto da IA. Motivo: esses números não têm nome de ninguém — o `A24` esconde os números **de cada pessoa**, e isso não muda — e o total ajuda a entender o volume do dia. Nada muda no código. O que o achado N-39 mostrou de errado (o seletor de liga da Caixa contava o setor inteiro e a lista mostrava só os itens da pessoa) foi corrigido à parte: a contagem segue o que cada papel vê. Se a equipe achar que o total confunde, a opção (b) — só os números dos próprios itens — continua disponível. |
| A59 | Modelo padrão da máquina da IA local: `qwen2.5:1.5b-instruct-q4_K_M`, **provisório** *(25/09/2026)* | **Decisão do dono**, depois da medição na máquina Debian (`A56 (d)`), feita pela sessão do Claude Code instalada nela e trazida por ele em capturas de tela. **Hardware medido:** Intel i5-3330 (4 núcleos, 3,0 GHz, 2012, **sem AVX2**), 7,6 GB de RAM (~6 livres), 428 GB de disco, sem GPU utilizável; Debian 12, sem `sudo`. **Gabarito 1.0.0, 17 casos sintéticos** — nota geral / só as respondidas: `qwen2.5:1.5b` **0,76** / 0,86; `llama3.2:3b` 0,65 / 0,85; `qwen2.5:3b` 0,61 / **0,94**. O `qwen2.5:3b` falha **sempre** em 4 casos (`documentos-sem-dados`, `ligantes-tres`, `ligantes-dois-tracos`, `duvida-liga`: JSON fora do esquema, também na segunda tentativa do adapter); os outros 2 que falharam na 1ª rodada eram instabilidade. Mesmo contando esses dois, ficaria em ~0,71. O 1.5b é ~2× mais rápido que os de 3B (cada chamada do 3b leva ~90 s) e ocupa ~1,4 GB. **Ponto fraco do 1.5b: literalidade 0,69** — reescreve em vez de copiar o texto do e-mail; um valor reescrito é exatamente o valor que pode não estar no e-mail (ver pendência 17 do `ESTADO.md`). Uma vez entrou em laço (`ligantes-tres`) e foi cortado pelo teto de 300 s do adapter — falhou alto, como devia. **O que esta decisão é:** o `IA_MODELO` do `.env` daquela máquina, para testar. **O que ela não é:** licença para dado real — `IA_PARA_DADO_REAL.local` continua `false` (`A56 (e)`). **Por que provisório:** (i) ~~janela de contexto não registrada~~ — **corrigido em 25/09, noite, lendo a conversa de lá inteira:** a janela era **8192** e o maior pedido do gabarito tem ~1.164 tokens (instruções de 2.125 caracteres em tempo de execução — o arquivo tem 2.147 porque guarda as quebras de linha como CR+LF, e o JavaScript normaliza cada quebra dentro do texto para uma só + forma esperada + e-mail). A hipótese de corte por janela está **refutada**; nesta máquina Windows, com Ollama 0.34.4 na janela de fábrica (4096), os pedidos têm ~1.060 tokens e o log do servidor diz `truncated = 0`. **A causa real das falhas do 3b, lida no log de lá: `itens: invalid_type; pareceInstrucao: invalid_type`** — ele responde HTTP 200 com um JSON de outra forma. (ii) o 3b tem literalidade perfeita quando acerta o formato; forçar a forma no servidor (esquema JSON no pedido, em vez de só `json_object`) pode inverter a escolha. Reavaliar com os dois pontos medidos. **Odysseus:** não instalado. Recomendação da sessão de lá, que confirma o `A56`: para o SBP ele não traz ganho (não deixa o modelo mais rápido nem melhor e disputa a RAM); só valeria como tela de chat para uso pessoal. **Configuração deixada na máquina:** Ollama e Node 24.21.0 em `~/.local`; serviço de usuário `systemd` com *linger* (sobe no boot sem login), só em `127.0.0.1:11434`, acesso pelo IP da rede recusado (conferido lá); modelo carregado por 1 h depois da última chamada; os três modelos continuam baixados (~3,9 GB os outros dois). A cópia do código é um **tarball** (sem git), baixado do repositório enquanto ele estava público — ver `ESTADO.md`. Achados da sessão de lá sobre o gabarito: corrigidos no PR "gabarito sem banco" (ver `ESTADO.md`). |
| A60 | O sistema é só para computador; não há versão de celular *(25/09/2026)* | **Decisão do dono:** "não precisamos criar uma versão para mobile, será unicamente para desktop." **O que muda:** tela de celular deixa de ser critério de aceite e de pendência; "visto rodando" é em largura de computador; nenhum trabalho novo de layout para celular. **O que não muda:** o que já foi feito para celular (cartões da Caixa no celular, botão de 44 px, `N-03`/`N-04`) fica — tirar por tirar é mexer no que está de pé (raio mínimo); vira código sem exigência. Acessibilidade continua valendo por inteiro: teclado e leitor de tela não são coisa de celular. **Pendências afetadas:** a 2 (explicação só em `title`) perde o motivo "toque", mantém teclado e leitor de tela; o `AT-46` (dois toques no Concluir) vira dois cliques, mesmo raciocínio. A convenção genérica "cartões no lugar de tabelas no mobile" não se aplica a este sistema. **Textos vencidos, marcados no mesmo PR:** `02-PRD.md` RNF-04 e a história "Minha Fila no celular", `03-SPEC.md` tabela de telas ("Mobile-first, cards"). |
| A62 | Jev (TypeSafe AI) entra como **segunda opinião**, atrás de uma camada de defesa do dado; sem roteador *(26/09/2026)* | **Decisão do dono:** integrar o Jev "das melhores maneiras possíveis em tudo no que ele se enquadrar", aproveitando o custo baixo, com pesquisa a fundo para não haver alucinação. **O que o Jev é** (fonte primária: os tipos do SDK oficial `@typesafe-ai/sdk` 0.6.0, publicado por contas `@typesafe.ai`): não gera texto; recebe um texto (`state`) e perguntas fechadas — sim/não (`noul`), escolha entre rótulos (`choice`) e nota numa escala (`score`) — e devolve probabilidades e confiança; `POST /v1/systemone`, `Authorization: Bearer`, modelo padrão `jev-latest`. **Por isso não substitui a interpretação** (achar nome, CPF, campos continua com a IA atual e com código — invariante 2). **Onde entra:** segunda opinião da categoria e da suspeita (discordância → Revisão humana), triagem em modo sombra, assistente escolhendo o verbete do manual (mostra o texto do manual, sem inventar) e alarme de concordância. **Direção mantida:** sem roteador dinâmico — nada troca de modelo sozinho; uso que mude o fluxo (ex.: pular a IA local) só depois de medido no gabarito e aprovado pelo dono. **Dado real:** não há acordo empresarial com a TypeSafe, então todo texto passa por `core/seguranca/protecao-para-fornecedor-externo.ts`: normaliza (NFKC, sem caracteres invisíveis) e tira link (com ou sem esquema, mesmo colado a outra palavra ou entre aspas), e-mail, número de conselho/matrícula/registro (mesmo com UF e até três palavras quaisquer no meio — aí, só a partir de 3 dígitos), bloco `data:…;base64` e toda sequência de 5+ dígitos com até três separadores — o que inclui data completa, valor longo e listas de números (`AT-49`); depois corta. Esta camada **substitui, para o classificador, o marcador reversível do `A52`** (o Jev não precisa devolver o CPF), e herda dele a condição de medir antes: o ganho do Jev se mede no gabarito, com dado sintético. **O que fica no texto:** nome, endereço e data por extenso, dado de saúde (CID — sensível, LGPD art. 11), placa, número curto com a palavra-chave depois (`Nº 1234 do CRM`) ou com mais de três palavras entre as duas (`CRM do Estado de São Paulo 1234`), domínio sem caminho, link partido por quebra de linha, dado de funcionário em assinatura e de terceiro em e-mail encaminhado, e escrita feita para escapar. Por isso a chave de dado real nasce **desligada**, e a condição do `A38` (termos que não usem o conteúdo para treinar) é **pré-requisito** para ligá-la — sem contrato, a TypeSafe não fica vinculada como operadora (LGPD art. 39). As condições exatas são pergunta ao dono: `§ H.4` item 35. **Não confirmado na fonte** (documentação, contrato e API bloqueados pela rede da sessão de 26/09): preço, qualidade em português, **uso do conteúdo para treino**, retenção fora do plano empresarial, país onde o dado fica (transferência internacional, art. 33), subprocessadores, exclusão a pedido, lista de espera. **Sem dependência nova:** chamada por `fetch` com a resposta validada por Zod, como em `ia-local.ts`. Status: 🟡 fase 1 (camada de defesa) no PR; porta, adaptador e usos em PRs próprios. |
...
31. **As respostas do gabarito da IA estão certas para a operação?** *(17/09/2026, `AT-36`)* O gabarito tem 17 e-mails inventados, cada um com a resposta que o sistema deveria dar (tipo do pedido, quantos itens, que dados copiar). Quem escreveu as respostas foi o agente, lendo as definições. Exemplo: *"Mudei de endereço e de telefone, alterem meus dados"* foi marcado como **ficha de cadastro**, não como dúvida geral. **A pergunta:** alguém da equipe pode conferir a lista (uma página, meia hora) e dizer onde responderia diferente? **Recomendação:** sim, antes de usar a nota para escolher o modelo local.
32. **Quantas tentativas antes de desistir de um e-mail, e "mandar a uma pessoa" é isso mesmo?** *(23/09/2026, achado C-11/N-13, `AT-41`)* Corrigido o problema técnico — o e-mail que a IA nunca consegue estruturar parava de cobrar 20 minutos antes de existir, e cobrava a cada sincronização até então. A correção desiste depois de **3** tentativas (hipótese, `§ C`) e marca o e-mail como tratado: ele some da fila de retentativa e vira um número na tela de Distribuição — *"N não puderam ser entendidos pela IA depois de várias tentativas — abra-os direto no Outlook"* —, do mesmo jeito que `naoLidas` e `repetidas` já funcionam ali. **Duas perguntas:** (a) três tentativas é pouco, muito, ou depende de quanto uma chamada de IA custa hoje (`A54`) para valer a pena mais uma repetição? (b) "aparecer como número na tela de quem sincroniza, com instrução de abrir no Outlook" é `mandar a uma pessoa` o bastante, ou o volume real vai pedir uma fila própria (visível a operador/gestor, com o e-mail já teria sumido da fila de retentativa) em vez de só um contador? **Recomendação:** manter 3 até haver custo medido com IA paga (`A49`); e começar pelo contador simples — criar fila nova é decisão de escopo, não deveria nascer de uma suposição.

33. ~~**Quando a conta trava por senha errada, a tela diz "espere 30 segundos" ou só "e-mail ou senha incorretos"?**~~ **RESPONDIDA em 24/09/2026 — ver `§ A57`: opção (b).** *Texto original:* *(24/09/2026, achado C-18)* Hoje, depois de 5 senhas erradas, a tela diz *"Muitas tentativas. Esta conta volta a aceitar entrada em 30s."*. Isso ajuda a Ana Sintética, que esqueceu a senha: ela sabe que é só esperar. Mas também ajuda quem está de fora: alguém tenta 6 vezes `ana.sintetica@…` e 6 vezes `joao.inventado@…`; para a Ana aparece "espere 30s", para o João (que não existe) aparece sempre "e-mail ou senha incorretos". Com isso ele descobre quem tem acesso ao sistema, sem nunca acertar uma senha. **(a)** manter como está — a equipe entende na hora, e quem sonda descobre a lista de contas; **(b)** mostrar sempre "e-mail ou senha incorretos" e, abaixo, uma frase fixa para todos: *"Errou várias vezes? Espere um minuto antes de tentar de novo."* — ninguém descobre nada, e a Ana ainda sabe o que fazer; **(c)** mensagem genérica e nada mais — mais seguro, e a Ana liga para o suporte. **Recomendação: (b).** Sem resposta, fica (a), como hoje. O que já foi corrigido sem depender disto: a conta travada passou a demorar o mesmo tempo que as outras recusas para responder (antes respondia em 5 ms, contra 250 ms — o relógio entregava a mesma informação).

34. ~~**O colaborador vê os totais do setor?**~~ **RESPONDIDA em 24/09/2026 — ver `§ A58`: opção (a).** *Texto original:* *(24/09/2026, achado N-39)* Hoje, quem é colaborador vê no Painel os números do **setor inteiro** por categoria (abertos, concluídos, pendentes) e a taxa de acerto da IA. Não há nome de ninguém nesses números — o `A24` escondeu os números **de cada pessoa**, mas não disse nada sobre os totais. Exemplo: a Bianca abre o Painel e vê "Ligante: 47 abertos", embora só 3 sejam dela. **Opções:** (a) continua como está — o total ajuda a pessoa a entender o volume do dia; (b) o colaborador vê só os números dos próprios itens; (c) o colaborador não vê o Painel. **Recomendação:** (a), porque não expõe ninguém e dá contexto; (b) se a equipe achar que o total confunde. *Já corrigido sem depender desta resposta:* o seletor de liga da Caixa contava itens do setor inteiro e a lista embaixo mostrava só os da pessoa ("· 47" e três itens); a contagem agora segue o que cada um vê.
...
**A nota:** quantidade de itens, categorias (sem ordem, dividido pelo maior dos dois tamanhos), campos esperados com valor literal, literalidade (valor devolvido que não está no e-mail derruba) e suspeita (nos dois sentidos). Média simples das que se aplicam. Falha de um caso vale zero; ao lado sai a nota só das respondidas, porque em 17/09 o Gemini respondeu 503 em 7 de 17 casos (nota geral 0,58, com as respondidas quase perfeitas). Título, observação e confiança ficam de fora: texto livre pediria outro modelo como juiz; calibrar confiança pede amostra maior. Fornecedor indisponível (`InterpretacaoIndisponivelError`) e erro de código param a avaliação em vez de virar nota.

**Linha de base:** o `IaMock` tira **0,92** e erra exatamente os casos sem palavra-chave (`*-sem-palavra-chave`, `documentos-sem-dados`) e a injeção que o detector por padrão não conhece (`injecao-sutil`). Fixada caso a caso em `src/servicos/avaliacao-da-ia.test.ts`: mudar o mock ou o gabarito muda o número de propósito.

**Hipóteses:** (a) as **respostas esperadas foram escritas pelo agente**, a partir das definições do prompt — não pela equipe; (b) peso igual entre as dimensões, embora errar a categoria (item na fila errada) custe mais do que errar um campo; (c) 17 casos bastam para comparar fornecedores de forma grosseira, não para calibrar limiar. **Impacto se estiverem erradas:** um modelo pode ser escolhido por acertar um gabarito que a operação responderia diferente — por isso `§ H.4` item 31. Mudar caso ou peso sobe `VERSAO_DO_GABARITO`; nota de versões diferentes não se compara. **Status:** ⏳ adotado; reavaliar quando a equipe conferir os casos e quando houver revisões reais suficientes (`qualidade-ia.ts`).
...
- **Contagem por `EventoProcessamento`, não uma tabela nova.** Cada falha de interpretação já gravava `situacao: 'reprocessavel'` com `referencia: messageId` — a trilha já sabia quantas vezes um e-mail tinha falhado, só ninguém perguntava. `contarTentativasAnteriores` conta essas linhas, em lote para o lote inteiro (mesma razão de `consultaDeProcessados`, ao lado): um `IN` por e-mail seguraria a leitura em centenas de idas ao banco.
- **Teto de 3 tentativas, hipótese provisória** (`TENTATIVAS_MAXIMAS_DE_INTERPRETACAO`, `§ C`, pergunta em `§ H.4` item 32). `ia-estruturada.ts` já tenta corrigir o formato sozinha uma vez; três sincronizações são três chances além dessa.
- **Na tentativa que estoura o teto, a IA nem é chamada.** É o que zera a cobrança — não "cobra e desiste depois", é "não cobra mais". O e-mail é gravado com `processadoEm` preenchido e zero itens: a partir daí `jaExiste?.processadoEm` do laço de `sincronizar` barra qualquer nova tentativa, do mesmo jeito que barra um e-mail comum já processado.
- **A análise de conteúdo suspeito continua rodando, sem custo.** `prepararConteudoExterno` é regex local, sem rede — desistir de chamar o MODELO não significa desistir de olhar o texto. O e-mail que estourou o teto ainda pode acender `conteudoSuspeito`, e esse sinal vai para o evento gravado.
- **Novo contador na tela, não nova fila.** `ResumoIngestao.naoInterpretados` aparece em `/distribuicao` do mesmo jeito que `naoLidas` e `repetidas` já aparecem: "N não puderam ser entendidos pela IA depois de várias tentativas — abra-os direto no Outlook". Cria fila própria (visível a operador/gestor, com o e-mail e o motivo) foi considerado e adiado — é decisão de escopo sobre o quanto isto vai acontecer na prática, que só uso real vai responder (`§ H.4` item 32b).

**O que fica diferente de antes:** um e-mail que a IA nunca consegue ler custa, no máximo, 3 tentativas de IA — nunca mais — e sempre aparece para alguém, em vez de sumir silenciosamente depois de 7 dias.

**Impacto se a hipótese do `3` estiver errada:** poucas tentativas de menos manda gente ao Outlook por um e-mail que a quarta chamada teria resolvido; de mais continua pagando por mais tempo. Ajustar é mudar uma constante, sem migração.
...
- **O que acontece acima do teto:** o item **não some**. Fica sem liga e vai para a revisão (a menção fica no payload, quem revisa vê o nome). Quem manda para a revisão é o desdobramento: barrar exige mais de um item no e-mail, e mais de um item sempre passa por humano — o teste exige que **todo** item sem liga esteja em revisão, e fica vermelho se essa regra mudar. Um evento `reprocessavel` registra quantas menções foram barradas, com o `messageId` — só quando alguma foi; alcançar 3 sem barrar nada não gera evento. Esse evento **não** conta para o teto de desistência do `AT-41` (lá só conta evento com a causa `falha_de_interpretacao`).
- **Corrigido ao terminar e na revisão técnica do PR #89:** a versão de 22/09 marcava com `anomalia` todo item cuja menção não virasse liga — inclusive `""` ou `"-"`, que modelo pequeno devolve no lugar de `null` (a revisão viraria a fila principal; o teste `menção vazia ou só pontuação não manda o item para a revisão` ficou vermelho contra ela). A revisão mostrou depois que esse ramo **nunca executava** (o desdobramento tem prioridade) e que o evento disparava com exatamente 3 ligas legítimas. O ramo saiu; o evento passou a depender de menção barrada.
...
**Hipótese:** a região viva (`Anuncio` em `componentes/matrizes.tsx`) só recebe **literais do código e números**: posição, contagem, nome de ação. Nunca título, assunto, remetente, texto que a IA escreveu lendo um e-mail, nem texto digitado por outra pessoa do sistema (nota do setor, motivo de devolução, título editado) — nenhum deles é a voz do sistema.
...
**Status:** 🟡 provisória **só na forma do aviso** — se a equipe, no teste com NVDA (pendência 6), achar a posição insuficiente para se orientar, a forma muda. **A proibição é firme** e não cai por isso: texto de fora só entra truncado, normalizado, **depois** da instrução, com rótulo falado ("título: …"), e com decisão registrada aqui. `textos-da-tela.test.ts` recusa `setFeito` com qualquer coisa além de `null` ou frase fixa.

### AT-49 — A camada de defesa tira data completa e valor longo *(26/09/2026)*

**Hipótese:** o classificador externo (`A62`) não precisa de data completa, de valor com 5 ou mais dígitos, nem de lista de números pequenos para dizer a categoria ou a suspeita de um pedido. `vencimento 30/09/2026`, `2026-09-26`, `anos 2025-2026`, `R$ 350,00`, `itens 1, 2, 3, 4, 5` e `14:30-15:00` viram `[número]`; ano sozinho (`2026`), valor curto (`R$ 35,00`) e contagem **de até 2 dígitos** depois de palavra-chave (`registro de 3 dependentes`) ficam; com 3 ou mais, a contagem depois de palavra-chave sai (`registro de 150 associados`, `registro em 2026`), porque a regra que pega `CRM da médica: 1234` não sabe distinguir os dois.
...
**Achados das duas revisões do PR #77, corrigidos com teste visto vermelho:**
- **ALTO (técnica):** `trocarSenha` não devolvia a tentativa reservada no ramo de hash ilegível — cinco tentativas de trocar a senha contra um hash corrompido trancavam a conta pelo mesmo defeito que o N-36 elimina, só que por outra rota.
- **MÉDIO (segurança):** naquele estado, as duas defesas contra força bruta sumiam juntas — `conferirSenha` decide pelo formato, antes do scrypt (custo de CPU zero), e o ramo devolve a tentativa (conta nunca trava). E cada tentativa gravava uma linha numa trilha sem expurgo. Agora o custo de derivação é pago também aqui, e o EVENTO sai no máximo uma vez por hora por pessoa; o log continua saindo sempre. O estado não é provocável de fora: ele depende do hash **gravado**, nunca da senha enviada.
- **MÉDIO (segurança):** o `DELETE` na trilha era garantia só de texto. Virou conferência: `src/servidor/privilegios.ts` (função pura, lê `SHOW GRANTS`), `npm run db:privilegios` (relata em desenvolvimento, **recusa** em produção ou com `EXIGIR_PRIVILEGIO_MINIMO=sim`) e `docs/03-SPEC.md § 14`, com o SQL de concessão mínima. `TRIGGER` na tabela também é negado: quem pode criar a trava pode derrubá-la e reescrever depois.
- **BAIXO (segurança), recusado com motivo:** voltar a resposta de credencial ilegível para `422` com a mensagem genérica de entrada. Quem está diante da tela precisa saber que o problema não é a senha que digita — senão tenta até ser trancado, que é o defeito inteiro do N-36 (invariante 7). Não é oráculo: o estado não é provocável de fora, o id não cruza para o cliente e o piso de tempo é pago.
