...
| A61 | O servidor da implantação é **Linux**; o Windows é só a máquina de desenvolvimento *(25/09/2026)* | **Decisão do dono:** "minha máquina é Windows e estamos fazendo os testes aqui, porém o projeto que estamos construindo para a empresa será em Linux." Até aqui isto era só "provavelmente" (`AT-32`). **O que muda:** quando Windows e Linux divergirem, **vale o Linux** — o CI (Ubuntu + MySQL 8.4) é a referência, não a máquina de desenvolvimento; o `AT-32` deixa de ser palpite e as armadilhas dele (conferência de schema com falso alarme, migração gerada em minúsculas) ficam como problema **só de quem desenvolve no Windows**, nunca do servidor. Caminho de arquivo, chave de anexo (`N-25`, sempre POSIX), agendamento e scripts se escrevem para Linux; caminho com `C:\` só em teste e em comentário que registra defeito. Medido na mesma data: a suíte inteira verde em Linux (1379 de 1379, sessão em nuvem, MySQL 8.4). **O que não muda:** a máquina da IA local continua a Debian (`A59`), à parte do servidor. **A confirmar com o TI da associação** (junto da credencial, `A47`): a distribuição e a versão, e o `lower_case_table_names` do MySQL de lá (esperado: `0`, o padrão do Linux). |
...
**Consequência prática, e é ela que importa:** esta conferência **não é confiável em Windows**, e o servidor da associação será Linux (`A61`, decisão do dono em 25/09/2026), onde o padrão preserva a caixa. **O resultado que vale é o do CI**, que roda em Linux — é lá que o passo existe para pegar o caso real: alguém edita o schema e esquece de gerar a migração.
