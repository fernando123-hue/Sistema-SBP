...
| A42 | Banco de dados da implantação: **MySQL** *(15/09/2026)* | **O sistema em uso vai rodar em MySQL**, e não em PostgreSQL, que era a hipótese registrada desde o começo (SPEC § 11, `AT-16`, comentários do `schema.prisma` e do `.env.example`). O protótipo continua em SQLite enquanto não houver implantação. **Nada no domínio muda:** não usamos `enum` nem `Json` nativos, os valores fechados são `String` validados por Zod, e os serviços não escrevem SQL específico — a única consulta crua (`servicos/painel.ts`) usa identificadores e tipos portáveis. **O que a troca exige de verdade está em `§ AT-28`**, e o item mais caro não é o provider: as 14 migrações existentes foram geradas para SQLite e **não servem** para MySQL. |
...
8. **Onde os testes rodam.** A suíte hoje sobe SQLite, e é isso que a mantém em segundos. Trocar tudo para MySQL tornaria o ciclo lento; rodar só em SQLite deixaria os itens 2, 3, 5 e 6 sem prova. **A decisão é de engenharia, e precisa ser tomada:** o caminho provável é a suíte continuar em SQLite e o CI ganhar um trabalho extra contra MySQL, cobrindo migração, colação, único com nulo e a consulta crua.
