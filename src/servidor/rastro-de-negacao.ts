 ...
 * pendência 8). `tentativa` é sempre texto fixo do código: a operação do
 * vocabulário fechado (`OperacaoSchema`) ou uma frase como "concluir item de
 * outra pessoa". NUNCA o corpo da requisição nem o caminho com ids.
 *
 * A falha ao gravar não troca a resposta por outra coisa: quem recebe a recusa
 * continua recebendo a recusa; o defeito do registro vai para o log.
 */
export async function registrarNegacao(
  banco: Banco,
  quem: { colaboradorId: string; papel: string },
  tentativa: string,
): Promise<void> {
  const contexto = { colaboradorId: quem.colaboradorId, papel: quem.papel, operacao: tentativa }
